...
如果 input 里有 `trend_intel_block` 字段,这是 **Gemini Search 实时拉取**的当前小红书真实热门帖子(标题 + 第一句 snippet),针对你这个产品品类。**这不是趋势分析,是原文样本**。
...
⚠️ **不要把 trend_intel_block 当装饰**。这是 Gemini 实打实搜来的"用户当前正在看什么",信号比你脑内对小红书的认知新鲜得多。把它作为你设计方向的**第一性输入**,然后再叠加底层方法论。
