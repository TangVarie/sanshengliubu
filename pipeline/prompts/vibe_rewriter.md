...
**三态决策表**(LLM 按 cell 逐个判断,**不要混淆**):

| 该 cell 的 `reference_packs_by_platform[cell.platform]` 状态 | 锚点来源 | `rewrite_summary` 必写 | 备注 |
|---|---|---|---|
| 非空,且与本 cell 主题/severity 匹配 | 从证据包挑 1 条做主锚点 | `源:数据库样本 #<id>(...原因...)` | id 用证据包 `id` 字段 |
| 非空,但全部已被本批次其他 cell 占用 | 选剩下的次优证据包,或退到 FALLBACK | 如证据包:`源:数据库样本 #<id>(...)`<br>如静态:`源:静态兜底 #<编号>(原因:该平台证据包已耗尽)` | 不能两个 cell 共用同一个 id |
| 空(该平台 0 命中) | 使用本 prompt 的静态兜底样本 | `源:静态兜底 #<编号>(原因:该平台 0 命中)` | **不报错、不略过**,继续重写 |

`rewrite_summary` 字段的"源:..."标记是上游审计抓飞轮 ROI 的唯一信号——**不写或写错,运营就以为飞轮没在工作**。
...
## 修复哲学（最高优先级，凌驾于下面所有改写技巧之上）

修复一句被判不合格的句子时，**先找它原本想说的那件事，用普通句子把事说出来**。

三条硬禁：

1. **禁止寻找另一种漂亮句式替换。** 把「不是省钱，而是省心」改成「与其说是省钱，
   不如说是省心」——机扫过了，读者没过。你只是把 A 指纹换成了 B 指纹。
   正确做法是删掉「不是X」那半句直接说，或者把对立拆成两个先后发生的时刻
   （"一开始图便宜买的。用了两周才发现省的是别的事"）。

2. **禁止用固定语调词替换逻辑连词。** 把「所以」批量换成「你看」「说白了」
   是用一套语调模板换掉一套逻辑模板，两边都是指纹，而且后者更难被发现。
   段落推进要靠**内容接住上一段留下的问题**，不靠信号词。

3. **禁止把机审命中的词换成同义词了事。** 「效果显著」改成「效果明显」没有
   解决任何问题——问题不在这四个字，在于这句话没说任何具体的事。
   改法是把它替换成一个具体到能想象出画面的事实（用了多久、什么变化、
   在什么场景下注意到的）。

**为什么这一段排在最前面**：这条流水线的机审闸（prose_gate）是纯正则的，
它只能判断"命中没命中"，判断不了"改得对不对"。如果你用换句式的方式过闸，
机扫会给你放行，然后这批稿子会带着更隐蔽的模板感发出去——那比原来那版更糟，
因为原来那版至少还能被机扫抓住。
