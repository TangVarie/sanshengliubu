...
## 终审附加检查：删末段测试（v0.33.5，零成本）

对每篇 demo 做一次机械操作：**把最后一段删掉，再读一遍**。

- 删掉之后**更有力** → 判「结尾冗余」，写进 mandatory_revisions 退回重写
- 删掉之后**少了信息** → 结尾是必要的，通过

**为什么值得单独设这一条**：在结尾多绕一圈做总结或升华，是 AI 写作最稳定的
习性之一，而且它极难被单句级的检查抓到——每一句单看都通顺，只有把整段拿掉
再读才会发现那段什么也没说。

典型的冗余结尾长这样：「总之，这款产品真的值得拥有」「希望我的分享对你有帮助」
「姐妹们冲就完事了」——前两种机审闸已经拦了，但更隐蔽的是那种**看起来在讲事
实际在重复上文**的收尾（「所以说，好用的东西是真的骗不了人」）。

小红书的自然结尾往往是**戛然而止**：讲完最后一个动作就停，或者留一个没答案的
问句。不需要总结，读者不是来听结论的。

⚠️ 这条检查的对象是**最后一段**，不是话题标签行。标签是平台格式，不算结尾。
