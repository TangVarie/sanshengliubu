...
VERSION_NOTES = (
    ...
    "静态评审一条都没提。"
    "\n\n"
    "【现象】用户传了一份 4.01MB 的样稿 Word,太子 tab 的文件清单上显示"
    "「Word · 已提取文本 · 33 字」。那 33 字是报错本身:"
    "`[DOCX 过大跳过: xxx.docx > 1024KB]`。文档里 7,827 字、12 篇样稿"
    "一个字都没进 brief,而整条 run 照常跑完 —— 六部拿到了业务背景,"
    "语感语料是空的。"
    "\n\n"
    "【根因 1:上限量错了对象】体积上限 1MB 是为了防「文本撑爆上下文」,"
    "却卡在**整个 zip 容器**上。docx/pdf 里绝大部分是图片字节,和它能吐出"
    "多少文字几乎无关 —— 那份文档 16 张配图占 4.4MB,正文只有约 8KB。"
    "于是上限拦掉的不是「文本太长」而是「文档里有图」。现在容器给 30MB 的"
    "读入上限,**提取出来的文本**再单独按 1MB 截断(截断而不是整份丢弃:"
    "超限时前半部分仍然有价值)。"
    "\n\n"
    "【根因 2:清单把失败显示成成功】文件清单按扩展名推断状态,而"
    "extract_file_content 失败时返回的占位符字符串照样是 body 的全部内容,"
    "于是 `[DOCX 过大跳过]` 被判成「Word · 已提取文本」。清单的唯一职责就是"
    "回答『收到没』,**它撒谎比它不存在更糟** —— 用户正是因为看到绿色才放心"
    "跑完了一整条 run。现在识别全部失败占位符(过大跳过/解析失败/无文字层/"
    "不支持类型)标红并在清单顶部报警,另加 partial 档给「截断了」和"
    "「有图没提」。"
    "\n\n"
    "【顺带修掉的三个同类洞】(1) docx 只取 `doc.paragraphs`,**不含表格**"
    "—— 而产品资料里的硬事实(成分/规格/价格带/竞品对比)基本全在表格里,"
    "丢得静默。现在按文档顺序提取段落 + 表格(表格转 Markdown)。"
    "(2) 内嵌图不转写但**必须明说**,不然用户以为图进去了;附带告诉他"
    "「导出成单独图片上传会走 Vision」。(3) 扫描版 PDF 没有文字层,PyPDF2"
    "安静返回空串,用户只看到「已提取 0 字」不知道该干嘛,现在明确提示。"
    "\n\n"
    "【结构性修复:删掉平行实现】page 3 的请旨补充资料原本是**另一套**上传"
    "处理:自己写一份体积上限常量,而且对一切非图片文件直接"
    "`data.decode(\"utf-8\")` —— 上传框明明收 pdf/docx,补一份 Word 进来就是"
    "一大坨替换字符塞进请旨回复。两边合并进 utils/file_extract。"
    "**这正是上一轮(v0.34.2 第 5 条)刚记过的教训:已有的实现就用,不要写"
    "平行的第二份。隔了一个版本又在另一个文件上犯了同一件事。**"
    "\n\n"
    "【方法论】四轮静态评审 34 条,没有一条碰到这个 bug —— 因为它不在代码"
    "逻辑里,在「上限该量哪个量」这个建模判断里,而且只有真喂一份图文并茂的"
    "文档才暴露。这条正好给 v0.35.0 那句「第一条真实 run 的价值高于再来一轮"
    "静态审查」补了个实证。"
    "\n\n"
    "v0.35.0 release: 质量度量与成本治理落地版 —— 收敛 v0.34.0 ~ v0.34.2,"
    "即 v0.33.x 九次迭代 + 四轮评审 34 条修复(10 → 13 → 6 → 5)。"
    "\n\n"
    "⚠️ **这是第一个准备真跑的版本。** 四轮评审全部是静态的,而本轮新增的三条热路径"
    "(断点重续 / 取消 / 限流)恰恰只有真跑才验得了 —— 四轮下来每轮都还能带出真问题,"
    "已经足够说明静态验证在这三条路径上的覆盖能力有多弱。首条 run 的验证顺序:"
    "正常跑一遍拿基线 → 中途点强制取消看是否真停 → 点继续执行看日志有无 [resume] 跳过。"
    "这条 run 的两个数字(红线通过率 / 高分篇数)同时也是回归哨兵的第一个基线点,"
    "哨兵要攒够 3 条 run 才开始报警。"
    "\n\n"
    "下面按版本倒序是这一轮的全部细节。"
    "\n\n"
    "v0.34.2 fix: 处理 Codex 第四轮评审的 5 条,全部属实、全部已修。"
    "四轮下来 10 → 13 → 6 → 5。这一轮最该记的是第 5 条:"
    "\n\n"
    "【在新代码路径上把一个已修的 bug 重新引入了一遍】批量采样为了不和外层重试叠乘,"
    "关掉了内层 call_with_retry 自己接管重试 —— 而我**手写**了一个瞬时错误分类器,"
    "只看 429/5xx/timeout。可 llm_retry._is_transient 里早就写着:欠费和账号停用也走"
    "429 外皮,v0.32.5 就是专门修这个的。手写的平行实现等于把那个补丁从这条路径上"
    "撤了 —— 余额不足会被当限流,60 个样本每个空等 2+4 秒重试三次。"
    "**已有的分类器就用,不要写平行实现;复制行为比复制代码更容易漏掉补丁。**"
    "\n\n"
    "另外 4 条按类型:"
    "① 请旨恢复仍有 TOCTOU 窗口 —— 上一轮把取消检查挪到了接受回复之前,但"
    "「读状态」和「写 running」之间还是有间隙,强制取消落在里面就会被无条件的 update "
    "改回 running。改成条件 UPDATE(try_resume_from_paused,只在状态确实还是 "
    "paused_for_review 时才切),判断交给数据库,窗口消失。和 try_claim_project_running "
    "同一个模式。**读-判-写的竞态,补检查点只能缩小窗口,消除它得靠数据库的条件写。**"
    "② 结构审的 resume 标记写早了:模型半边挂掉时(未配置/限流/解析失败)确定性前置审"
    "照样出 hint,矩阵看着「有结构审结果」,但模型能抓的那类(合规写成空话)一条没查 ——"
    "标记一写,续跑永久跳过整个结构审。现在阶段函数返回「模型半边是否真跑成」,失败不写标记。"
    "③ 策略升级的 refined_c 快照用「plan 变没变」判断是否跑完 —— 但 secretariat 改完锚点、"
    "重跑 vibe_loop 抛异常时 plan 已经是新的了,矩阵却还是按旧锚点改写的那份。"
    "改成升级函数显式返回 completed。**「某个对象变了」不能代替「这一步跑完了」,"
    "中途失败恰恰会留下变了一半的状态。**"
    "④ 回归哨兵把两条指标的基线条数 max 成一个数给 UI,而两条基线是**两批不同的 run**"
    "(红线不筛覆盖度、高分层只用覆盖完整的),UI 的文案于是把「覆盖完整」安到了红线头上。"
    "去掉合成字段,两条分开展示。"
    "\n\n"
    "【结论不变】四轮静态审查每轮都带出真问题,resume / 取消 / 限流三条路径只有真跑"
    "才验得了。第一条真实 run 的价值仍然高于再来一轮静态审查。"
    "\n\n"
    "v0.34.1 fix: 处理 Codex 第三轮评审的 6 条,全部属实、全部已修。"
    "三轮下来 10 → 13 → 6,条数在降,但**同一类错误一直在复发**,值得单独记:"
    "\n\n"
    "本轮 6 条里,有 2 条是【我修「恢复 ≠ 重跑」时又犯了「恢复 ≠ 重跑」】——"
    "refined_c 快照没带被策略升级清理过的画像反应(恢复后陈旧反应复活,而对应 cell "
    "早被重写)、恢复升级后的 plan 时没重建派生的 _direction_index(消费者模拟拿旧"
    "stop_trigger 评判新 cell)。另有 2 条是【修复本身不完整】:画像双 backend 全失败"
    "时照写 completed 标记(resume 永久跳过,哪怕原因只是限流)、合成批准只改了"
    "output_data 没改 status(而 _load_completed_stages 只认 completed,等于没落库)。"
    "还有 1 条是【我引入的退化】:关掉内层 call_with_retry 之后用宽泛 except 接管重试,"
    "丢掉了原来的瞬时/永久分类 —— 没配 key、模型名错、400 这类永不自愈的错误,每个样本"
    "都要空等 2+4 秒失败三次,60 个样本叠起来会拖住线程池。最后 1 条是竞态:请旨等待里"
    "取消检查放在了接受回复之后,两个标签页同时回复+取消时,取消会被静默吃掉。"
    "\n\n"
    "【结论】静态验证在 resume / 取消 / 限流这三条路径上的覆盖能力,比我预期的低得多。"
    "三轮 py_compile 全绿 + pyflakes 干净 + 逐情形对拍,仍然每轮都带出新问题(本轮"
    "pyflakes 还抓到我自己引入的一个重复字典键)。这三条路径的共同点是**只有真跑才"
    "验得了**。第一条真实 run 的价值高于再来几轮静态审查。"
    "\n\n"
    "v0.34.0 release: 质量度量与成本治理 —— 收敛 v0.33.0~v0.33.8 九次迭代 + "
    "两轮 Codex 评审的 23 条修复。"
    "\n\n"
    "【起因】一次全流水线审查发现一个结构性缺口:交付物是给**批量生成**用的 "
    "system_prompt(要产出 N≥10 篇),但 11 道质量闸看的全都是每个 cell 唯一那篇 "
    "demo_output。后果有三:①决定'第 7 篇会不会和第 3 篇一个味'的 5 池 + 人设轮换"
    "机制从来没被验证过(唯一的检查是数一数池子的文字在不在 prompt 里,存在 ≠ 有效);"
    "②n=1 测的是 demo 的质量不是 prompt 的分布(那篇 demo 被打磨过最多 3 轮);"
    "③优化方向是反的 —— 目标是上限(100 篇里 5 篇爆款 > 100 篇都 85 分),而最贵的"
    "两个环节干的恰恰是把唯一样本往均值推。"
    "\n\n"
    "【新增能力】quality_metrics 双层评分(红线追 100%、高分层追绝对数翻倍且有意"
    "不提供 rate —— 当 pass rate 优化会压掉方差=压掉爆款);batch_sampler 批量采样"
    "(真跑 N 篇只变 seed,首次能回答'5 池轮转到底生没生效');prose_gate 机审闸"
    "(翻案句/黑话/伪精确 + 跨篇指纹,抓结构重合而非词面重合);回归哨兵(跟项目"
    "历史中位数比,掉出噪声带自动报警)。"
    "\n\n"
    "【成本治理】终审输入 allowlist 瘦身(104K→54K 字符,砍 49%);终审换 "
    "deepseek-v4-pro(输入省 86%、输出省 94%);消费者模拟只跑 weak 档;画像弱判据"
    "取交集;叙事导演重建限流;成本表校准(v4-pro 输出侧原估算高估 138%)。"
    "\n\n"
    "【稳健性】断点重续补全(此前工部构建之后整条精炼链无 resume,约 79-98 次调用"
    "全部重跑 —— 保护做在了错误的一半);取消检查点(此前 force_cancel 只翻数据库"
    "状态,点了取消按钮解锁了、钱还在烧);采样纳入主限流器(此前 60 次采样绕开限流,"
    "打出的 429 让主链路误以为自己太快而减半速率)。"
    "\n\n"
    "【方法论上的三次自我纠正,写下来是因为都容易重犯】"
    "① 第一版成本排序按【调用次数】排,结论是'红蓝精炼最贵'。按 token 重算完全"
    "不成立,真正的钱在终审的 k3 输入。**按次数排序会系统性高估小提示词高频调用、"
    "低估大输入低频调用。**"
    "② v0.33.5 给吏部加的三个人设字段死在了 works.md 的固定字段表上 —— 而'三层"
    "摘要漏斗'正是本轮最初审查里自己点名批评过的问题。**发现一类问题不等于免疫"
    "这类问题,需要的是机制不是记性。**"
    "③ v0.33.8 的断点重续功能自己带出了一整批二阶缺陷(第二轮评审 13 条里占大半),"
    "归纳成一句:**恢复 ≠ 重跑**。一个阶段的副作用只要不止'往 final_system 挂一个"
    "字段',查 done 跳过就会漏掉其余副作用,而且漏得静默。最严重的一处是修订重跑"
    "没作废新加的快照 —— 用户点「应用修订意见」会拿到被静默丢弃修订的旧稿。"
    "**resume 和 revision 是同一份 stage_log 的两个读者,只加读的那一半必出事。**"
    "\n\n"
    "⚠️ 全部验证为逻辑级和单元级,**未跑端到端**。断点重续/取消/限流三项都是新加"
    "在热路径上的,而那三条路径恰恰只有真跑才验得了 —— 逻辑验证在这里覆盖能力最弱。"
    "建议首条 run:正常跑一遍拿基线 → 中途点强制取消看是否真停 → 点继续执行看日志"
    "有无 [resume] 跳过。"
    "\n\n"
    "v0.33.8 perf: 断点重续补全 + 取消变成真取消 + 采样纳入限流 + 辩论早收敛,"
    "外加终审换 deepseek-v4-pro 与成本表校准。"
    "(1)【断点重续只覆盖了前半条链】此前 resume 只查太子/中书省/尚书省/六部/"
    "工部架构,外加格子规划和工部构建的 cell 级恢复 —— **工部构建之后的整条精炼链"
    "没有任何 resume**。12 格子在终审挂掉后点「继续执行」,叙事导演/红蓝/画像×2/"
    "结构审/网感循环/消费者模拟/批量采样约 79-98 次调用会全部重跑。保护做在了"
    "错误的一半:单次最贵的工部构建有 cell 级恢复,调用量最大的后半条链完全裸奔。"
    "修法按阶段是否改写 prompt_matrix 分两类:不改的(画像/结构审/消费者模拟/采样)"
    "查 done 直接跳过并恢复诊断字段;改的(叙事导演+红蓝、网感循环)打矩阵快照 ——"
    "存整个 prompt_matrix 而不是 diff(diff 要处理 cell 增删和字段级合并,复杂度和"
    "出错面都远大于直接存一份;12 格子约 50KB,一条 run 最多两次快照,用 100KB 写入"
    "换掉 80 次 LLM 调用的重跑)。批量采样那 60 次是单项最大收益。"
    "(2)【取消不是真取消】force_cancel_pipeline 的 docstring 自己写明了 Python "
    "杀不掉挂起的守护线程,所以它只翻数据库状态解开 UI 的锁 —— 点了取消之后"
    "按钮解锁了、钱还在烧,僵尸线程会跑到自己结束或 TCP 超时。全流水线没有任何"
    "取消检查点。现在 7 个 stage 边界各查一次 run 状态,不是 running/"
    "paused_for_review 就抛 PipelineCancelled 走正常退出路径;查询失败一律当作"
    "未取消继续(宁可漏一次取消,不要因为一次 DB 抖动误杀正常 run)。取消走独立的"
    "except 分支,不写 traceback marker —— 用户自己点的取消不该显示成一堆吓人的"
    "堆栈。⚠️ 只能在两次调用之间生效,已发出的 HTTP 请求仍要等它返回。"
    "(3)【采样绕开限流器打自己人】kimi_client 不走主链路的 _SlidingWindowLimiter,"
    "这对二审/结构审那种一两次调用的岗位没问题,但采样是 60 次。两条链共用同一个"
    "上游配额却只有一条自律:采样打出的 429 会让主链路的自适应限流器误以为【自己】"
    "太快而减半速率 —— 主链路被一个它管不着的旁路拖慢。现在采样在 slot() 下发出,"
    "限流器不可用时(Vertex 模式)退回裸调用。"
    "(4)【辩论早收敛】门下省某轮一条 challenge 都没提就直接收敛,不再走完剩下的"
    "「中书省修订 → 门下省再审」完整往返 —— 中书省每轮要重吐完整大 plan(跑 k3 的"
    "$15/1M 输出档,这条链上最贵的部分),换回来的多半是同一份方案。只在非末轮生效。"
    "(5)【终审换档 + 成本表校准】chancellery_final 从 kimi-k3($3/$15)换成 "
    "deepseek-v4-pro($0.42/$0.84,2026-08-17 新价),输入省 86% 输出省 94%。理由:"
    "终审吃的是全流水线最大的一份输入,而产出是结构化评审 JSON 不是中文文案 ——"
    "K3 的中文写作优势在这个岗位上用不上;两者同为 1M 上下文旗舰档;顺带保住了"
    "'终审跟工部不同厂家'这条性质。同时校准成本表:v4-pro 此前记的是估算值"
    "$0.5/$2.0,实价 $0.42/$0.84,输出侧高估了 138% —— 正是这个高估让它在之前的"
    "选型里显得比实际贵一倍多,估错的单价会直接导出错的结论。"
    "⚠️ 有意**不**把 v4-pro 铺到内容生成侧:第三方中文文案评测里 kimi-k2.6 在"
    "「AI 味最低、最接近人类写作」这一项领先,而这条流水线卖的就是这个。要换也该"
    "先用 batch_sampler + quality_metrics 做 A/B 拿数字说话,不是拍脑袋。"
)
_VERSION_NOTES_V0337 = (
    "v0.33.7 feature: 回归哨兵 —— 自动发现'某次改动之后质量悄悄掉了'。"
    "(1) 要解决的不是'记得打开某个开关'(四个 P3 flag 默认就是开的,不需要人记),"
    "而是【靠记性做不到的那件事】:没人会盯着每条 run 的两个数字去跟上周对比。"
    "(2) 做法:每条 run 算完分数后,跟同项目最近 8 条 run 的中位数比,掉出噪声带"
    "就写红字告警。新增 db.get_quality_score_history(跨 run 查 quality_score,"
    "分两步查避免 PostgREST 嵌套过滤的不确定行为 + 控 payload)。"
    "(3) 两条指标口径不同:红线通过率是确定性判定、不受 temperature 影响,容差 2% "
    "基本等于'只要跌就报';高分篇率受生成抖动影响,要跌出 15% 噪声带才算(≈12 格"
    "里差 1.8 篇,和小样本实测抖动同量级)。"
    "(4) 高分层跨 run 比用【率】不用绝对数 —— 格子数会变(方向数 × 平台数不固定,"
    "策略升级还会加格子),绝对数跨 run 不可比。这和 UI 上'看绝对数翻倍'不矛盾:"
    "那是同口径纵向对比,这里是跨口径的。"
    "(5) 基线只取【评分覆盖完整】的历史 run。critic 挂过的那几条 run 高分篇数是"
    "被低估的,混进基线会把标准拉低,于是真正的退步反而报不出来。本轮自己覆盖不全"
    "时也不判高分层。"
    "(6) 历史不足 3 条时不报警 —— 前几条 run 是在建立基线。"
    "(7) 告警在产出中心单独给位置,不塞进 strategic_warnings 的 expander:那个面板"
    "按 cell_id + root_cause_explanation 渲染,而哨兵告警是 run 级的,塞进去会显示成"
    "一行空白。"
)
_VERSION_NOTES_V0336 = (
...
# 长上下文判断档 — 1.6T / 1M 上下文 / $0.42 in · $0.84 out(2026-08-17 新价)。
# 用途:输入极大、但要的是【判断】不是【中文写作风格】的岗位。
#
# 引入它的直接原因是终审:chancellery_final 吃的是全流水线最大的一份输入
# (整个 prompt_matrix),原来跑 kimi-k3($3/$15)。两者都是 1M 上下文的旗舰档,
# 而终审输出的是结构化评审 JSON、不是要给人读的中文文案 —— K3 的中文写作优势
# 在这个岗位上用不上,却要付 3.6 倍输入价和 17.9 倍输出价。
#
# ⚠️ 有意**不**把它铺到内容生成侧。第三方中文文案评测里 kimi-k2.6 在
# 「AI 味最低、最接近人类写作」这一项领先,而这条流水线卖的就是这个 ——
# 用便宜五倍的模型换掉产品的核心卖点,账不该这么算。要换也该先用
# batch_sampler + quality_metrics 做 A/B 拿数字说话,不是拍脑袋。
LONGCTX_JUDGE_MODEL = "deepseek-v4-pro"
...
KIMI_DEEPSEEK_MAP: dict[str, str] = {
    ...
    # 终审(holistic 把关)。v0.33.8: kimi-k3 → deepseek-v4-pro。
    # 它吃的是全流水线最大的一份输入,而产出是结构化评审 JSON 不是中文文案,
    # K3 的写作优势在这里用不上。两者同为 1M 上下文旗舰档,单价差一个量级。
    # 顺带保住了"终审跟工部不同厂家"这条性质(工部是 Kimi 系)。
    "chancellery_final": LONGCTX_JUDGE_MODEL,
...
COST_PER_1M_INPUT: dict[str, float] = {
    ...
    # v0.33.8 按官网实价校准(2026-08-17 生效的新价:¥3/1M in、¥6/1M out,
    # 按 ~7.1 CNY/USD 折算)。此前是 $0.5 的**估算值**,高估了约 19% ——
    # 而这个数会直接影响成本回显和选型判断,估错了会得出错的结论。
...
COST_PER_1M_OUTPUT: dict[str, float] = {
    ...
    # ── 历史 run 回显用,同上 ────────────────────────────────────────
    # 输出侧修正幅度大得多:旧估算 $2.0 → 实价 $0.84,高估 138%。
    # 正是这个高估让 v4-pro 在之前的选型里显得比实际贵一倍多。
