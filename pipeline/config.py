...
# v0.33.0 ~ v0.33.8 是同一轮改造的九次迭代,已整体合入 main。这里收敛成一个
# 发布号,下面是这一轮到底做了什么的总账;逐版细节仍保留在 _VERSION_NOTES_V033x。
VERSION_NOTES = (
    "v0.34.0 release: 质量度量与成本治理 —— 合并 v0.33.0~v0.33.8 九次迭代。"
    "\n\n"
    ...
    "②n=1 测的是 demo 的质量不是 prompt 的分布(那篇 demo 被红蓝 + 网感重写打磨过"
    "最多 3 轮);③优化方向是反的 —— 目标是上限(100 篇里 5 篇爆款 > 100 篇都 85 分),"
    "而最贵的两个环节干的恰恰是把唯一样本往均值推。"
    "连锁效应:验收信号只有 n=1 → 每道闸置信度都低 → 只能靠堆闸门数量找安全感 →"
    "11 道闸互相重叠、判决又都建立在同一篇样本上。"
    "\n\n"
    "【新增能力】(1) quality_metrics 双层评分:红线层追 100% 通过率,高分层追高分篇"
    "**绝对数**翻倍且有意不提供 rate 字段(当 pass rate 优化会压掉方差=压掉爆款);"
    "(2) batch_sampler 批量采样:拿最终 system_prompt 真跑 N 篇、只变 {{seed}},"
    "首次能回答'5 池轮转到底生没生效';(3) prose_gate 机审闸:翻案句/商业黑话/"
    "伪精确 + 跨篇指纹(抓结构重合,和已有查重抓的词面重合互补);(4) 回归哨兵:"
    "跟同项目历史中位数比,掉出噪声带自动报警。"
    "\n\n"
    "【成本治理】终审输入 allowlist 瘦身(12 格子实测 104K→54K 字符,砍 49%);"
    "终审换 deepseek-v4-pro(输入省 86%、输出省 94%);消费者模拟只跑 weak 档;"
    "画像弱判据取交集;叙事导演重建限流;成本表校准(v4-pro 输出侧原估算高估 138%)。"
    "\n\n"
    "【稳健性】断点重续补全 —— 此前工部构建之后的整条精炼链没有任何 resume,"
    "12 格子约 79-98 次调用会全部重跑(保护做在了错误的一半:单次最贵的工部构建有"
    "cell 级恢复,调用量最大的后半条链完全裸奔);取消检查点 —— force_cancel 此前"
    "只翻数据库状态解开 UI 锁,点了取消之后按钮解锁了、钱还在烧;采样纳入主限流器 ——"
    "此前 60 次采样绕开限流,打出的 429 会让主链路误以为自己太快而减半速率。"
    "\n\n"
    "【方法论上的两次自我纠正,写下来是因为容易重犯】"
    "① 第一版成本排序按【调用次数】排,结论是'红蓝精炼最贵'。按 token 重算完全"
    "不成立(红蓝提示词才 1848 字符、红队找不到问题就跳过蓝队),真正的钱在终审的"
    "k3 输入。**按次数排序会系统性高估小提示词高频调用、低估大输入低频调用。**"
    "② v0.33.5 给吏部加了三个人设字段,但它们死在 works.md 重建 persona_library 的"
    "固定字段表上 —— 而'三层摘要漏斗'正是本轮最初审查里点名批评过的问题。"
    "**发现一类问题不等于免疫这类问题,需要的是机制不是记性。**"
    "\n\n"
    "⚠️ 全部验证为逻辑级和单元级,**未跑端到端**。断点重续/取消/限流三项都是新加"
    "在热路径上的,真实行为需实测。建议首条 run:正常跑一遍拿基线 → 中途点强制取消"
    "看是否真停 → 点继续执行看日志有无 [resume] 跳过。"
)
_VERSION_NOTES_V0338 = (
