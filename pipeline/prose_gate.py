...
# ── 伪精确行为量 ──────────────────────────────────────────────────────
# 「停留了 1.7 秒」「看了 3.5 分钟」—— 真人不会这样记自己的行为。
#
# ⚠️ v0.33.6 收窄:单位表从「时间 + 次数」缩到**只有秒/分钟/小时**。
#
# 原来还收 天/周/年/次/遍,那是错的 —— 这些单位有大量**合法的产品事实**用法:
#   「质保 1.5 年」「临床随访 2.5 年」「每天 1.5 次」「疗程 2.5 周」
# 而本规则自己的理由写的就是"产品规格属事实层、照常精确",收这些单位等于
# 自相矛盾,还会把有出处的事实硬推去重写。
#
# 秒/分钟/小时带小数在小红书语境下几乎只可能是编出来的行为量:没有哪个
# 产品规格用「1.5 秒」标注,也没有人真的记得自己犹豫了 2.3 分钟。
# 其余单位降到 SOFT 档(见 _PSEUDO_PRECISE_SOFT),记标不 fail。
PSEUDO_PRECISE_PATTERN = re.compile(
    r"\d+\.\d+\s*(?:秒|分钟|小时)"
)
# 软档:这些单位带小数**可能**是伪精确,也可能是合法产品事实(质保 1.5 年 /
# 每天 1.5 次),交给 critic 结合上下文判断,机审只记一笔。
_PSEUDO_PRECISE_SOFT = re.compile(
    r"\d+\.\d+\s*(?:天|周|个月|年|次|遍|顿|口)"
)
...
def scan_text(text: str, *, duplicate_opening: bool = False) -> dict[str, Any]:
    ...
    m_soft = _PSEUDO_PRECISE_SOFT.search(body)
    if m_soft:
        soft.append({
            "rule": "pseudo_precise_maybe", "value": m_soft.group(0),
            "detail": f"{m_soft.group(0)!r} —— 可能是伪精确行为量,也可能是"
                      "合法产品事实(质保 1.5 年 / 每天 1.5 次)。"
                      "结合上下文判断,机审不判死",
        })
